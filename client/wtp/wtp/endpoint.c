...
static wtp_status_t wtp_handle_open(
    ...
    WIO_TRY(wtp_begin_pkt(tx_ctrl, WTP_PKT_ACK))
    WIO_TRY(wtp_end_pkt(tx_ctrl))
...
static wtp_status_t wtp_handle_msg_packet(
    ...
    for (uint8_t i=0;i<n_msgs;i++) {
        ...
        wio_callback_t* recv_cb = self->_recv_cb;
        uint8_t recv_cb_begin = self->_recv_cb_begin;
        //Get next receive callback
        if ((recv_cb_begin<self->_recv_cb_end)||(recv_cb[recv_cb_begin])) {
            //Set callback function and closure data
            cb = recv_cb[recv_cb_begin];
            cb_data = self->_recv_cb_data[recv_cb_begin];

            //Clear callback item
            recv_cb[recv_cb_begin] = NULL;
            //Update queue begin
            self->_recv_cb_begin++;
            if (self->_recv_cb_begin>=self->_recv_cb_size)
...
wtp_status_t wtp_init(
    ...
    //Receive callbacks memory
    uint8_t* recv_cb_mem = malloc(n_recv_cb*(sizeof(wio_callback_t)+sizeof(void*)));
    if (!recv_cb_mem)
        return WIO_ERR_NO_MEMORY;
    //Receive callbacks and closure data
    self->_recv_cb = (wio_callback_t*)recv_cb_mem;
    self->_recv_cb_data = (void*)(recv_cb_mem+n_recv_cb*sizeof(wio_callback_t));
    for (uint8_t i=0;i<n_recv_cb;i++)
        self->_recv_cb[i] = NULL;
    //Queue begin and end
    self->_recv_cb_begin = self->_recv_cb_end = 0;
    //Queue size
    self->_recv_cb_size = n_recv_cb;
...
wtp_status_t wtp_connect(
    ...
    WIO_TRY(wtp_begin_pkt(tx_ctrl, WTP_PKT_OPEN))
    //End packet
    WIO_TRY(wtp_end_pkt(tx_ctrl))
...
wtp_status_t wtp_recv(
    ...
    //Receive callbacks and closure data
    wio_callback_t* recv_cb = self->_recv_cb;
    void** recv_cb_data = self->_recv_cb_data;
    //Receive callback queue begin and end
    uint8_t recv_cb_begin = self->_recv_cb_begin;
    uint8_t recv_cb_end = self->_recv_cb_end;

    //No space available to add callback
    if ((recv_cb_begin==recv_cb_end)&&recv_cb[recv_cb_begin])
        return WIO_ERR_NO_MEMORY;

    //Add callback to end of the queue
    recv_cb[recv_cb_end] = cb;
    recv_cb_data[recv_cb_end] = cb_data;

    //Update queue end
    self->_recv_cb_end++;
    if (self->_recv_cb_end>=self->_recv_cb_size)
        self->_recv_cb_end = 0;
 ...
 * {@inheritDoc}
 */
wtp_status_t wtp_fill_read_mem(
    wtp_t* self
) {
    //TODO: Fill read memory
...
wtp_status_t wtp_fill_epc_mem(
    ...
    //Clear EPC memory
    memset(epc_buf->buffer, 0, epc_buf->size);
    //Reset EPC buffer
